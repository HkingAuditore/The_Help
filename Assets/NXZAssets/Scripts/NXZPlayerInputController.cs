...
namespace NXZ
{
    [RequireComponent(typeof(NXZCharacterMotor))]
    public class NXZPlayerInputController : MonoBehaviour
    {
        ...
        private NXZCharacterMotor motor;
        private CharacterController controller;
        private NXZPlayerState playerState;
        ...
        void Start()
        {
            motor = GetComponent<NXZCharacterMotor>();
            controller = GetComponent<CharacterController>();
            playerState = GetComponent<NXZPlayerState>();
            stamina = maxStamina;
            if (!mainModel)
            {
        ...
        void Update()
        {
            ...
            //Cancel Sprint
            if (sprint && (Input.GetAxis("Vertical") < 0.02f && Mathf.Abs( Input.GetAxis("Horizontal")) < 0.02f) || sprint && stamina <= 0 || sprint && Input.GetButtonDown("Fire1") || sprint && Input.GetKeyUp(KeyCode.LeftShift))
            {
                sprint = false;
                recover = true;
            ...
            //Activate Sprint
            if (Input.GetKey(KeyCode.LeftShift) && (Input.GetAxis("Vertical") > 0 || (Input.GetAxis("Vertical") == 0 && Input.GetAxis("Horizontal")!= 0)) && (controller.collisionFlags & CollisionFlags.Below) != 0 && canSprint && stamina > 0)
            {
                sprint = true;
